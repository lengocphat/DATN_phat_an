...
class hr_job(osv.Model):
    ...
    _columns = {
        ...
        'state': fields.selection([('open', 'Draft'), ('waiting', 'Waiting HRM'), ('recruit', 'Recruitment in Progress')],
    ...
    def set_waiting(self, cr, uid, ids, context=None):
        for job in self.browse(cr, uid, ids, context=context):
            ...
            self.write(cr, uid, [job.id], {'state': 'waiting', 'no_of_recruitment': no_of_recruitment}, context=context)
        return True
...
class hr_employee(osv.osv):
    ...
    def onclick_contract_sign(self, cr, uid, ids, context=None):
        employee = self.browse(cr,uid,ids[0],context=None)
        if employee.join_date and employee.contract_type:
            contract_obj = self.pool.get('hr.contract_type')
            contract_data = contract_obj.browse(cr, uid, employee.contract_type.id)
            if not employee.end_date:
                end_date = date.today()
            else:
                end_date = datetime.strptime(employee.end_date, '%Y-%m-%d').date()
            last_end_date = end_date
            if contract_data:
                res = end_date + relativedelta.relativedelta(years=contract_data.year) + \
                                           relativedelta.relativedelta(months=contract_data.month) + \
                                           relativedelta.relativedelta(days=contract_data.day)
        else:
            raise Warning(u'Hãy điền đầy đủ thông tin!')
        return super(hr_employee, self).write(cr, uid, ids, {'end_date':res, 'last_end_date':last_end_date})
    ...
    _columns = {
        ...
        'phone': fields.char('Phone', size=11),
        'mobile': fields.char('Mobile', size=11),
        'email': fields.char('Email', size=128, help="Email"),
        'ssnid': fields.char('SSN No', help='Social Security Number'),
        'sinid': fields.char('SIN No', help="Social Insurance Number"),
        'identification_id': fields.char('Identification No', size=9),
        ...
        'work_phone': fields.char('Work Phone', size= 11, readonly=False),
        'mobile_phone': fields.char('Work Mobile', size= 11, readonly=False),
        ...
        'skills': fields.one2many('hr.employee_skill', 'emp_id', type='char', string='Technical Skills'),
        # 'tech_skill': fields.many2one('hr.technical_skill', 'Technical Skills'),
        # 'tech_skill_level': fields.many2one('hr.technical_skill_level', 'Skills Level'),
        ...
        'last_end_date': fields.date('End Date'),
    ...
    def create(self, cr, uid, data, context=None):
        ...
        try:
            resource_id = resource_obj.search(cr, uid, [('user_id', '=', data['user_id'])], context=None)
            if resource_id:
                raise Warning(u'Tài khoản này đã có người sử dụng')
        except:
            print 1
    ...
    def write(self, cr, uid, ids, vals, context=None):
        try:
            if vals['user_id']:
                resource_obj = self.pool.get('resource.resource')
                resource_id = resource_obj.search(cr, uid, [('user_id', '=', vals['user_id'])], context=None)
                if resource_id:
                    raise Warning(u'Tài khoản này đã có người sử dụng')
        except:
...
class technical_skill(osv.osv):
    """ Category of technical skill """
    _name = "hr.technical_skill"
    _description = "Category of technical skill"
    _columns = {
        'name': fields.char('Name', required=True),
        'employee_id': fields.one2many('hr.employee_skill', 'skill_id', type='char'),
    }
class technical_skill_level(osv.osv):
    """ Technical skill Experience"""
    _name = "hr.technical_skill_level"
    _description = "Technical skill Experience"
    _columns = {
        'name': fields.char('Name', required= True),
        'employee_id': fields.one2many('hr.employee_skill', 'skill_level_id', type='char'),
    }
class employee_skill(osv.osv):
    """ Employee skill"""
    _name = "hr.employee_skill"
    _description = "Employee Skill"
    _columns = {
        'emp_id': fields.many2one('hr.employee', type='char', string='Employee', invisible=True),
        'skill_id': fields.many2one('hr.technical_skill', type='char', string='Skill'),
        'skill_level_id': fields.many2one('hr.technical_skill_level', type='char', string='Skill Level'),
    }
