...
class project(osv.osv):
    ...
    def _progress_rate(self, cr, uid, ids, names, arg, context=None):
        ...
        if len(ids) > 1:
            for id in ids:
                task_count = 0
                task_complete = len(task_obj.search(cr, uid, (
                    [('is_parent_task', '=', False), ('project_id', '=', id), ('stage_id','=',7)]), context=context))
                task_count = len(task_obj.search(cr, uid, (
                    [('is_parent_task', '=', False), ('project_id', '=', id)]), context=context))
                if task_count != 0:
                    res[id]= round(100.0 * task_complete / task_count, 2)
                else:
                    res[id] = 0.0
            return res
        else:
    ...
    _columns = {
        ...
        'progress_rate': fields.function(_progress_rate, string='Progress', type='float', group_operator="avg", help="Percent of tasks closed according to the total of tasks todo.",
            # store = {
                # 'project.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                # 'project.task': (_get_projects_from_tasks, ['planned_hours', 'remaining_hours', 'work_ids', 'stage_id'], 20),
            ),
...
class task(osv.osv):
    ...
    def check_task_deadline(self, cr, uid, context=None):
        today = datetime.date.today()
        task_config = self.pool.get('project.task')
        stage_config = self.pool.get('project.task.color')
        due_soon_stages = stage_config.search(cr, uid, [('due_soon_deadline', '=', 'True')], context=None)
        stage_id = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'in progress')], context=None)
        tasks = self.search(cr, uid, [('stage_id', '=', stage_id[0])], context=None)
        ds_tasks = task_config.browse(cr, uid, tasks, context=None)
        if due_soon_stages:
            ds_stage = stage_config.browse(cr, uid, due_soon_stages, context=None)
            for stage in ds_stage:
                date_due_soon = today + relativedelta.relativedelta(days=stage.days) + relativedelta.relativedelta(
                    hours=stage.hours) + relativedelta.relativedelta(minutes=stage.minutes)
                if date_due_soon:
                    for ds_task in ds_tasks:
                        if ds_task.date_deadline:
                            deadline = datetime.datetime.strptime(ds_task.date_deadline, '%Y-%m-%d').date()
                            if date_due_soon >= deadline and deadline >= today:
                                super(task, self).write(cr, uid, ds_task.id, {'color':3}, context=None)
        overdue_stages = stage_config.search(cr, uid, [('deadline', '=', 'True')], context=None)
        if overdue_stages:
            ds_stage = stage_config.browse(cr, uid, overdue_stages, context=None)
            for stage in ds_stage:
                for ds_task in ds_tasks:
                    if ds_task.date_deadline:
                        deadline = datetime.datetime.strptime(ds_task.date_deadline, '%Y-%m-%d').date()
                        if deadline < today:
                            super(task, self).write(cr, uid, ds_task.id, {'color': 2}, context=None)
    ...
    #     res = super(task, self).read(cr, uid, ids, fields=fields, context=context, load=load)
    #     print(ids)
    #     #print res
    #     return res
    #     #return super(task, self).read(cr, user, ids, fields=fields, context=context, load=load)
    ...
    def check_is_dev(self, cr, uid, ids, field_name, arg, context=None):
        res = {}
        for task in self.browse(cr, uid, ids, context=context):
            res[task.id] = False
            if uid == task.user_id.id:
                res[task.id] = True
        return res
        # task_obj = self.browse(cr, uid, ids, context=None)
        # if uid == task_obj.user_id.id:
        #     self.is_dev = True
        # return True

    def check_is_ba(self, cr, uid, ids, field_name, arg, context=None):
        res = {}
        for task in self.browse(cr, uid, ids, context=context):
            res[task.id] = False
            if uid == task.assigned_to_ba.id:
                res[task.id] = True
        return res

    def check_is_qc(self, cr, uid, ids, field_name, arg, context=None):
        res = {}
        for task in self.browse(cr, uid, ids, context=context):
            res[task.id] = False
            if uid == task.assigned_to_qc.id:
                res[task.id] = True
        return res
    ...
    _columns = {
        ...
        'parent_task': fields.one2many('project.task', 'parent_task_name', string='Parent Task'),
        'is_parent_task': fields.boolean('Is Sub Task'),
        'parent_task_name': fields.many2one('project.task',string= 'Parent Task Name'),
        # 'par_assigned_to': fields.many2one('res.users',
        #                                   string='Assigned to',
        #                                   related='parent_task.user_id',
        #                                   default=lambda self: self.env.uid),
        'completed': fields.char('Sub Tasks Percent Completed'),
        # 'assigned_to_dev': fields.many2one('res.users',
        #                                   string='Assigned to DEV'),
        'assigned_to_ba': fields.many2one('res.users',
                                         string='Assigned to BA'),
        'assigned_to_qc': fields.many2one('res.users',
                                         string='Assigned to QC'),
        'support_dev': fields.many2many('res.users', string='Support Dev Tags'),
        'support_ba': fields.many2many('res.users', 'test_user_rel', 'user_id', 'task_ids', string='Support BA Tags'),
        'assigned_to_dev_stage': fields.selection([('4', ''), ('5', 'In Progress'), ('6', 'Done'),
                                                            ('7', 'Cancelled')], "Stage DEV", type='char'),
        'assigned_to_ba_stage': fields.selection([('4', ''), ('5', 'In Progress'), ('6', 'Done'),
                                                           ('7', 'Cancelled')], "Stage BA", type='char'),
        'assigned_to_qc_stage': fields.selection([('4', ''), ('5', 'In Progress'), ('6', 'Done'),
                                                           ('7', 'Cancelled')], "Stage QC", type='char'),
        'is_dev': fields.function(check_is_dev, type='boolean'),
        'is_ba': fields.function(check_is_ba, type='boolean'),
        'is_qc': fields.function(check_is_qc, type='boolean'),
        'task_for': fields.selection([('dev', 'DEV'), ('ba', 'BA'), ('qc', 'QC')], string='Task for', required=True),
    }
    _defaults = {
        ...
        'assigned_to_dev_stage': '4',
        'assigned_to_ba_stage': '4',
        'assigned_to_qc_stage': '4',
        'task_for': 'dev',
    }
    _order = "priority desc, sequence, date_start, name, id"

    def button_dev_done_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.user_id.id:
            task_data.assigned_to_dev_stage = '6'
            if task_data.assigned_to_ba_stage != '5' and task_data.assigned_to_qc_stage != '5':
                project_task_type_ids= self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'done')])
                if project_task_type_ids:
                    task_type_data = self.pool.get('project.task.type').browse(cr, uid, project_task_type_ids[0])
                    task_data.stage_id = project_task_type_ids[0]
                else:
                    raise Warning(_("project_task_type_ids don't have code = done!!!!"))

    def button_ba_done_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.assigned_to_ba.id:
            task_data.assigned_to_ba_stage = '6'
            if task_data.assigned_to_dev_stage != '5' and task_data.assigned_to_qc_stage != '5':
                project_task_type_ids = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'done')])
                if project_task_type_ids:
                    task_type_data = self.pool.get('project.task.type').browse(cr, uid, project_task_type_ids[0])
                    task_data.stage_id = project_task_type_ids[0]
    ...
    def button_qc_done_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.assigned_to_qc.id:
            task_data.assigned_to_qc_stage = '6'
            if task_data.assigned_to_ba_stage != '5' and task_data.assigned_to_dev_stage != '5':
                project_task_type_ids = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'done')])
                if project_task_type_ids:
                    task_data.stage_id = project_task_type_ids[0]
                else:
    ...
    def button_dev_progress_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.user_id.id:
            task_data.assigned_to_dev_stage = '5'
            project_task_type_ids = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'in progress')])
            if project_task_type_ids:
                task_data.stage_id = project_task_type_ids[0]
            else:
                raise Warning(_("project_task_type_ids don't have code = in progress!!!!"))

    def button_ba_progress_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.assigned_to_ba.id:
            task_data.assigned_to_ba_stage = '5'
            project_task_type_ids = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'in progress')])
            if project_task_type_ids:
                task_data.stage_id = project_task_type_ids[0]
            else:
    ...
    def button_qc_progress_click(self, cr, uid, ids, context=None):
        task_data = self.browse(cr, uid, ids)
        if uid == task_data.assigned_to_qc.id:
            task_data.assigned_to_qc_stage = '5'
            project_task_type_ids = self.pool.get('project.task.type').search(cr, uid, [('code', '=', 'in progress')])
            if project_task_type_ids:
                task_data.stage_id = project_task_type_ids[0]
            else:
    ...
    def onchange_is_par_task(self, cr, uid, ids, is_parent_task, context=None):
        if is_parent_task == False:
            return {'value': {'parent_task_name': ''}}

    def assigned_to_dev_onchange(self, cr, uid, ids, user_id, context=None):
        assigned_to_dev_stage = '4'
        if user_id:
            assigned_to_dev_stage = '5'
        if uid == user_id:
            return {'value': {'is_dev': True, 'assigned_to_dev_stage': assigned_to_dev_stage}}
        else:
            return {'value': {'is_dev': False, 'assigned_to_dev_stage': assigned_to_dev_stage}}

    def assigned_to_ba_onchange(self, cr, uid, ids, assigned_to_ba, context=None):
        assigned_to_ba_stage = '4'
        if assigned_to_ba:
            assigned_to_ba_stage = '5'
        if uid == assigned_to_ba:
            return {'value': {'is_ba': True, 'assigned_to_ba_stage': assigned_to_ba_stage}}
        else:
            return {'value': {'is_ba': False, 'assigned_to_ba_stage': assigned_to_ba_stage}}

    def assigned_to_qc_onchange(self, cr, uid, ids, assigned_to_qc, context=None):
        assigned_to_qc_stage = '4'
        if assigned_to_qc:
            assigned_to_qc_stage = '5'
        if uid == assigned_to_qc:
            return {'value': {'is_qc': True, 'assigned_to_qc_stage': assigned_to_qc_stage}}
        else:
            return {'value': {'is_qc': False, 'assigned_to_qc_stage': assigned_to_qc_stage}}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if task_data:
            ...
            if task_data.is_parent_task == False:
                if vals.get('parent_task'):
                    count_new_task = len(vals['parent_task']) - len(task_data.parent_task)
                    i = 1
                    for tasks in vals['parent_task']:
                        if i < len(vals['parent_task']):
                            if task_data.parent_task[task_count].stage_id.name == 'Done':
                                task_complete_count += 1
                            i += 1
                            if tasks[2] != False:
                                if tasks[2].get('stage_id'):
                                    if tasks[2].get('stage_id') == 7:
                                        task_complete_count += 1
                                    else:
                                        if task_data.parent_task[task_count - 1].stage_id.name == 'Done':
                                            task_complete_count -= 1
                        else:
                            tasks = tasks[2]
                            if tasks != False:
                                if tasks.get('stage_id'):
                                    if tasks.get('stage_id') == 7:
                                        task_complete_count += 1
                            i += 1
                            # super(task, self).create(cr, uid, tasks, context=context)
                        task_count += 1
                else:
                    for tasks in task_data.parent_task:
                        task_count += 1
                        if tasks.stage_id.code == 'done':
                            task_complete_count += 1
                if task_count != 0:
                    percent_complete_task = 100 * task_complete_count / task_count
                vals['completed'] = str(percent_complete_task) + '%'
        ...
        self.check_task_deadline(cr, uid, context=None)
        return result
...
class project_task_color(osv.osv):
    _name = 'project.task.color'
    _rec_name = 'color_name'

    _columns = {
        'color': fields.integer('Color Id', required=True),
        'color_name': fields.char('Stage', required=True),
        'due_soon_deadline': fields.boolean('Due soon deadline'),
    ...
    _sql_constraints = [
        ('color_unique', 'unique (color)', 'Color must be unique'),
        ('color_name_unique', 'unique (color_name)', 'Color name must be unique'),
        ('hours_check', 'CHECK( hours < 24 and hour >= 0 )', 'Hour must between 0 and 23'),
        ('minutes_check', 'CHECK( minutes < 60 and  minutes >= 0 )', 'Minutes must between 0 and 59')
    ]
