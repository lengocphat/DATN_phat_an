...
class hr_applicant(osv.Model):
    ...
    def _get_day(self, cr, uid, ids, fields, args, context=None):
        res = {}
        flag = date.today()
        today = datetime.strftime(flag, '%d-%m-%Y')
        list_time = today.split('-')
        for id in ids:
            res[id] = list_time[0]
        return res

    def _get_month(self, cr, uid, ids, fields, args, context=None):
        res = {}
        flag = date.today()
        today = datetime.strftime(flag, '%d-%m-%Y')
        list_time = today.split('-')
    ...
    _columns = {
        ...
        'email': fields.char('Email', size=128, help="Applicant's email.", required=True),
        'phone': fields.char('Phone', size=32),
        'mobile': fields.char('Mobile', size=32, required=True),
        'address': fields.char('Address', size=128, required=True),
        ...
        'partner_id': fields.many2one('hr.employee', 'Contact'),
        ...
        'technical_skills': fields.text('Technical Skills'),
        'social_skills': fields.text('Social Skills'),
        'experiences': fields.text('Experiences'),
        ...
        'national_id_number': fields.char('National ID Number', required=True),
        'join_date': fields.date('Join Date'),
        'end_date': fields.date('End Date'),
        'contract_type': fields.many2one('hr.contract_type', string='Contract Type'),
        'days': fields.function(_get_day, type='char'),
        'months':fields.function(_get_month, type='char'),
        'years':fields.function(_get_year, type='char'),
    }
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, context=None):
        data = {'partner_phone': False,
                ...
                'email_from': False}
        if partner_id:
            addr = self.pool.get('hr.employee').browse(cr, uid, partner_id, context)
            data.update({'partner_phone': addr.work_phone,
                        'partner_mobile': addr.mobile_phone,
                        'email_from': addr.work_email})
        return {'value': data}
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if applicant:
            dict_end_date = self.onchange_end_date(cr, uid, obj_id,applicant.join_date, applicant.contract_type.id)
            res = super(hr_applicant, self).write(cr, uid, obj_id, dict_end_date['value'], context)
        return obj_id

    # def  _get_end_date(self, cr, uid, ids, fields, args, context=None):
    #     res = {}
    #     for id in ids:
    #         app = self.browse(cr, uid, id, context)
    #         if app.join_date:
    #             join_date = datetime.strptime(app.join_date, '%Y-%m-%d').date()
    #             if app.contract_type:
    #                 end_date = join_date + relativedelta.relativedelta(years=app.contract_type.year) + relativedelta.relativedelta(months=app.contract_type.month) + relativedelta.relativedelta(days=app.contract_type.day)
    #                 res[id] = end_date
    #     return res

    def onchange_end_date(self, cr, uid, ids, join_date, contract):
        res = {}
        if join_date and contract:
            contract_obj = self.pool.get('hr.contract_type')
            contract_data = contract_obj.browse(cr, uid, contract)
            date_start = datetime.strptime(join_date, '%Y-%m-%d').date()
            if contract_data:
                res.update({
                    'end_date': date_start + relativedelta.relativedelta(years=contract_data.year) + \
                                           relativedelta.relativedelta(months=contract_data.month) + \
                                           relativedelta.relativedelta(days=contract_data.day)
                })
        return {'value': res}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        applicant_obj = self.browse(cr, uid, ids, context=None)
        stage = applicant_obj.stage_id.id
        salary_expected = 0
        salary_proposed = 0
        if stage == 4 or stage == 5:
            if vals.get('salary_expected', False):
                salary_expected = vals['salary_expected']
            else:
                salary_expected = applicant_obj.salary_expected
            if salary_expected <= 0:
                raise Warning('Expected salary must be > 0!!!')

            if vals.get('salary_proposed', False):
                salary_proposed = vals['salary_proposed']
            else:
                salary_proposed = applicant_obj.salary_proposed
            if salary_proposed <= 0:
                raise Warning('Proposed salary must be > 0!!!')
        ...
        flag = super(hr_applicant, self).write(cr, uid, ids, vals, context)
        if flag:
            app = self.browse(cr, uid, ids[0])
            dict_end_date = self.onchange_end_date(cr, uid, ids, app.join_date, app.contract_type.id)
            res = super(hr_applicant, self).write(cr, uid, ids, dict_end_date['value'], context)
        return res
    ...
    def create_employee_from_applicant(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        if self.browse(cr, uid, ids, context=context).stage_id.id != 5:
            raise Warning('Stage must be Contract Signed to Create Employee!!')
        ...
        for applicant in self.browse(cr, uid, ids, context=context):
            ...
            if applicant.job_id and (applicant.partner_name or contact_name):
                ...
                emp_id = hr_employee.create(cr, uid, {'name': applicant.partner_name or contact_name,
                                                     ...
                                                     'birthday': applicant.birthday or False,
                                                     'address_home_id': applicant.address or False,
                                                     'phone': applicant.phone or False,
                                                     'mobile': applicant.mobile or False,
                                                     'email': applicant.email or False,
                                                     'department_id': applicant.department_id.id or False,
                                                     'address_id': applicant.company_id and applicant.company_id.partner_id and applicant.company_id.partner_id.id or False,
                                                     'work_email': applicant.department_id and applicant.department_id.company_id and applicant.department_id.company_id.email or False,
                                                     'work_phone': applicant.department_id and applicant.department_id.company_id and applicant.department_id.company_id.phone or False,
                                                     'salary_proposed': applicant.salary_proposed or False,
                                                      'identification_id': applicant.national_id_number or False,
                                                     }, context=create_ctx)
...
class contract_type(osv.osv):
    _name = "hr.contract_type"
    _columns = {
        'name': fields.char('Name', required=True),
        'year': fields.integer('Year(s)'),
        'month': fields.integer('Month(s)'),
